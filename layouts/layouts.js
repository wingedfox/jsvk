...
VirtualKeyboard.addLayout({code:'ZH-CN',name:'Chinese Simpl. Cangjie',keys:'`1234567890-=\\手田水口廿卜山戈人心[]日尸木火土竹十大中;\'重難金女月弓一,./',shift: {0:'~!@#$%^&*()_+|QWERTYUIOP{}ASDFGHJKL:"ZXCVBNM<>?'},cbk:{'load' : function () {
     /*
...
//     VirtualKeyboard.Langs.CN.PYArrTradTone=VirtualKeyboard.Langs.CN.PYArrTradTone;
     }
,'activate' : function () {
...
VirtualKeyboard.Langs.JP = new function () {
	...
	for(var i=0; i < dbRom2Kana.length; i++){
            dbi=dbRom2Kana[i]
            dbi0=dbi[0]
		...
		if(dbi.length > 4) Rom[dbi0].push(dbi[3]+hiraVowel, dbi[4]+'ー')
		else if(dbi.length > 3) Rom[dbi0].push(dbi[3]+'ー')
                aChoon.push(dbi0)
